...
La seconda afferma che **scrivere codice e' l'esperienza utente del progetto**. Questa affermazione e' meno netta della prima, ed e' gia' parzialmente smontata nello stato corrente del progetto. Le macro `tweak` e la famiglia `edit-*` (sezione 11.2) sono modalita' di edit non testuale: l'utente muove slider, trascina nodi e maniglie di un gizmo nel viewport, o guida una turtle con la tastiera, e Ridley produce o modifica codice come effetto; il caso piu' esplicito e' `edit-attach` (11.2.3), dove ogni drag di una freccia o di un anello del gizmo diventa un comando turtle nel sorgente. Il viewport picking e il sistema di anchor `mark` (sezione 6.7) permettono di selezionare facce e punti dal viewport invece che riferirli per coordinate. L'integrazione AI (sezione 11.4), nei limiti di efficacia documentati in 15.4.5, traduce intenti naturali in codice. Tutte queste superfici **producono codice**, e quindi rispettano la prima affermazione. Ma allargano la seconda: l'utente che le usa non sta scrivendo codice nel senso letterale, sta interagendo con un'altra superficie che lo produce per lui.
...
**Persistenza.** In webapp tutto vive in `localStorage`: gli script (i *workspace*), le librerie utente come JSON, le settings AI, la history AI, i prompt store. In desktop la cartella `~/.ridley/libraries/` ospita le librerie utente come file `.clj` di testo (sezione 11.3.2), mentre il resto resta in `localStorage` anche in desktop (settings, history AI, prompt store). Il dual-backend dello storage librerie è uno dei pattern ricorrenti documentati in capitolo 14.

**Workspace vs librerie.** L'editor non ha più un singolo documento ma una sessione di *workspace* (`ridley.workspace.store`, chiavi `ridley:workspaces`/`ridley:current-workspace`), documenti di prima classe multipli e switchabili dal pannello "Workspaces". Sono un asse distinto dalle librerie: un workspace è lo script che si *esegue*, una libreria è una *dipendenza* caricata in SCI come namespace prefissato — i workspace non entrano mai nella lista delle librerie attive né in SCI. La persistenza è a due livelli: la *sessione* (localStorage) sopravvive a riavvio e prevale sul disco; su desktop un workspace può essere *legato* a un file (`:file-path`), con `:disk-content` per l'indicatore "modifiche non salvate" (Save scrive sul path legato, Save As ri-lega). I workspace ephemeral senza file non vengono mai scritti su disco. Aprire un esempio del manuale crea un nuovo workspace e ci passa sopra, senza toccare il lavoro corrente.

**Librerie ancorate al workspace.** Ogni workspace ricorda l'insieme di librerie che usa (campo `:libraries`). Le librerie restano un *asset globale condiviso* (una sola copia, in `~/.ridley/libraries/` o localStorage): la lista attiva globale (`get-active-libraries`, riletta a ogni run da `load-active-libraries`) è trattata come **proiezione del workspace corrente**. Due sincronizzazioni in `core.cljs`: quando un workspace diventa corrente si proietta la sua lista su `set-active-libraries!` (`apply-workspace-libraries!`); quando l'utente tocca le checkbox del pannello la lista globale viene ricopiata nel workspace (`capture-active-libraries!`). Le librerie mancanti sono già gestite dal caricatore (warning "not found" + skip; reimportate, tornano attive). La lista **viaggia col file**: al Save viene serializzata in una riga-header `;; Ridley Workspace — Libraries: ...` (`serialize-file`), strippata al load (`parse-file`) — stesso pattern dell'header delle librerie — così il body in CodeMirror resta solo codice. Non si mette mai `require` nel `.clj`.
...
La panoramica sopra omette diversi sottosistemi. In particolare non compaiono l'animation system, i pannelli di testo 3D (`panel`), il sistema di librerie utente, la voice interface e gli editor modali (tweak e la famiglia edit-*). Non si tratta di dimenticanze: sono tutti sottosistemi che si innestano su questa struttura principale (tipicamente sul DSL runtime o sulla scena) senza alterarne la forma. Li discute il capitolo 11, come sottosistemi ausiliari.
...
flowchart TB
    ...
    subgraph POST [" "]
        ...
        Feedback["audio/play-feedback!<br/>modal/enter! (se richiesto)"]
...
- **Trigger dei modal evaluator** (`tweak-start!`, `tweak-start-registered!`, `edit-attach-request!` e gli omologhi della famiglia `edit-*`; `pilot` sopravvive come alias macro di `edit-attach`): invocano le modalita' interattive documentate in 2.2.
...
Il path e' **comandi, non campioni**. I waypoint con posizioni e tangenti vengono calcolati piu' tardi, quando un consumer interpreta i comandi su una turtle. Questo replay e' oggi inline in ogni consumer: `loft-from-path` lo fa via `analyze-loft-path` piu' un loop su `apply-rotation-to-state` (in `src/ridley/turtle/loft.cljs`), `extrude-from-path` via `analyze-open-path-dir` con lo stesso pattern (in `src/ridley/turtle/extrusion.cljs`). L'helper condiviso che faceva questo lavoro, `walk-path-poses`, e' stato rimosso a maggio 2026 insieme a `bloft`. Il sample rate non vive nel path, vive nel consumer.

Conseguenza pratica: lo stesso path puo' essere campionato a risoluzioni diverse da consumer diversi senza ricostruirlo. Un loft con `loft-n 64` campiona piu' fitto di un default a 16; un extrude lineare fa un solo campione per waypoint. Il path non lo sa: e' una sequenza di istruzioni, e l'interprete decide come leggerla.

Dal 2026-07 questo principio vale anche per le curve: `bezier-to`, `arc-h`/`arc-v` e `bezier-as` entrano nel recording come **comandi analitici** (control point nel frame d'ingresso del segmento), non come tessellazione. La tessellazione è un servizio unico a valle — `lower-commands`, raggiunto dall'accessor `path-micro-commands` (`src/ridley/turtle/core.cljs`) — con un'eccezione dichiarata al principio del sample rate: gli step di tessellazione delle curve sono oggi risolti e congelati a record time (`:steps` sul comando, per compatibilità byte-identica con il comportamento storico); il passaggio alla resolution di consumo è l'ultimo item aperto del piano di migrazione, documentato in `dev-docs/high-level-recording-accertamento.md`.
...
Il `:recording` contiene i comandi al livello a cui l'utente li ha scritti; i micro-comandi tessellati esistono solo a valle di `lower-commands`, ed è su quello stream che vivono i **tag**: chiavi opzionali sulla mappa comando, protocollo privato della cucitura tessellazione→estrusione. Servono al consumer micro per distinguere un passo di tessellazione da un comando dell'utente — un `(th 15)` scritto a mano e un `th` di 15° emesso per seguire una corda sono identici per l'interprete della turtle, ma per l'estrusione significano cose opposte: un corner da mitrare contro una transizione liscia tra sezioni.

Chi scrive è sempre `lower-commands` (`src/ridley/turtle/core.cljs`); i lettori sono due, entrambi in `src/ridley/turtle/extrusion.cljs`:

| Tag | Chi lo scrive | Chi lo legge | Cosa garantisce |
|---|---|---|---|
| `:smooth true` | I passi di tessellazione dei bezier (`lower-bezier-to`, ramo auto di bezier-to-anchor) | La corner machinery: `corner-rotation?` = comando di rotazione **e** non `:smooth` | Passo di curva, non corner dell'utente: niente miter né shortening, transizione liscia tra sezioni |
| `:arc-cap :lead` / `:arc-cap :trail` | I mezzi passi con cui `lower-arc*` apre e chiude l'integrazione midpoint | `split-leading-cap` | Cap iniziale/finale dell'estrusione perpendicolare all'heading di ingresso/uscita (il guard rail-start non c'entra più: un arco alto livello è esente per costruzione) |
| `:bez-cap :lead` | Le rotazioni prima del primo `f` di un run bezier | `split-leading-cap` | Cap iniziale perpendicolare all'heading pre-finestra, escluso dalla misura a corda |

**La storia, in breve.** Fino a metà 2026 la tessellazione avveniva a record time, e una seconda famiglia di tag — di *ricostruzione* — riportava a valle l'informazione analitica che la registrazione distruggeva: `:pure`/`:span` (la curva contrabbandata al re-edit dell'editor) e `:veer-deg` (la tangente contrabbandata al guard rail-start). La migrazione al recording alto livello (`dev-docs/high-level-recording-accertamento.md`, Fasi 1–3, 2026-07) li ha rimossi insieme ai loro ultimi lettori: l'editor interpreta i comandi alto livello direttamente (3D e 2D), il guard calcola il veer dalle componenti locali di `:c1` (`curve-entry-veer-deg`). Grep di uscita a zero in `src/`.

Il `tr` tra i passi di tessellazione esiste perché la composizione th-poi-tv è una decomposizione Euler che non riproduce da sola il trasporto a rotazione minimale dell'up: il lowering chiude ogni passo con il roll residuo che porta l'up sul frame canonico della curva (`canonical-bezier-frame-impl` in `src/ridley/turtle/bezier.cljs` — la sorgente unica del frame lungo un bezier, campionata da lowering, turtle runtime ed editor; vedi `dev-docs/brief-bezier-canonical-frame.md`).

**Le asimmetrie residue, dichiarate apposta.** I passi ordinari degli archi e di `bezier-as` non sono taggati `:smooth`: per la corner machinery sono oggi sequenze di corner duri — è il motivo del dual-ring del loft sui rail ad arco (Roadmap, «qualità su rail curvo»); per `bezier-as` l'allineamento è un accertamento tracciato in `dev-docs/code-issues.md`. Sono elencate qui contro la trappola ricorrente del progetto: dedurre per analogia che due emettitori simili si comportino allo stesso modo. Questo capitolo è il posto dove l'analogia si verifica invece di presumersi.

La regola per chi aggiunge un tipo di curva: si estende `lower-commands` — la cucitura è una sola — dichiarando lì i tre momenti (lead prima del primo `f`, passi intermedi, trail dopo l'ultimo) rispetto a `:smooth` e ai tag di cap. Un momento non dichiarato viene letto dal consumer come intenzione dell'utente.

Ultima distinzione, contro le collisioni di nomi: questi tag vivono sui micro-comandi prodotti dal lowering. Non vanno confusi né con i flag a livello di path (`:bezier true`), né con gli `:spans` del source tracking (sezione 5.4) — lì si mappa il sorgente, qui si annota la geometria.
...
**Loft, loft-n, loft-between.** Le tre varianti dispatchano da `loft-impl` a tre funzioni pure distinte: `pure-loft-path` (shape singola lungo path), `pure-loft-two-shapes` (interpolazione fra due profili), `pure-loft-shape-fn` (profilo che varia con `t`). Camminano il path replayando i comandi su una turtle locale (loop inline su `apply-rotation-to-state`, vedi `analyze-loft-path` in loft.cljs) a sample rate fisso (default 16 step, configurabile via `loft-n`).

`loft-between` interpola fra due shape diverse lungo un path, costruendo ring che gradualmente si trasformano dall'una all'altra. La gestione degli angoli del path, comune a tutte le varianti, sta in `process-loft-corners`, che calcola offset di scorciatoia per profili rastremati e gestisce le giunture in base al `joint-mode`. `loft-from-path` emette piu' sub-mesh con vertex table indipendenti (sweep prima del corner, mesh d'angolo, sweep dopo, piu' i due cap); `pure-loft-path` in `editor/operations.cljs` le concatena e poi passa per `mesh-utils/merge-vertices` (epsilon 1e-4) che salda i ring di confine, chiudendo le cuciture e producendo una mesh watertight anche su angoli sharp. Su curvature talmente strette che i ring si auto-intersecano in 3D (specialmente con holes, dove il ring interno spazza un raggio piu' piccolo) il risultato puo' restare auto-intersecante: il rimedio sta a monte (smussare con `bezier-as`/`arc-h`, ridurre il raggio della shape).
...
La risoluzione del meshing e' governata da `*sdf-resolution*`, una dynamic var con default 15 voxel per unita'. `resolution-for-bounds` la converte in una griglia e applica un budget massimo di voxel per evitare meshing troppo costosi. Per SDF con feature sottili (piccoli dettagli rispetto alla scala generale), `min-feature-size` boosta automaticamente la risoluzione locale. La regione di meshing, quando non fornita esplicitamente, e' stimata da `auto-bounds` camminando l'albero per rami; dal 2026-07-09 il ramo `rotate` propaga l'AABB degli otto angoli ruotati dei bounds del figlio invece della sfera-bounding, eliminando l'inflazione composta ((√3)^n con n nodi rotate annidati) che affamava il mesher sulle catene di rotazioni prodotte dagli attach con stretch e cp-rotazioni.
...
flowchart LR
    subgraph DSL["DSL runtime - editor/"]
        ...
        IM["interactive-mode<br/>sessioni modali<br/>test-state"]
    end
...
**Un secondo viewport, generalizzato a N istanze** (`ridley.viewport.inset`, nato in brief-acquisition-views.md Parte 1, generalizzato in brief-mesh-board-views.md Parte 2) rompe l'assunzione "un solo `WebGLRenderer`/`Scene`/`Camera`": ogni istanza è un piccolo overlay picture-in-picture con un proprio canvas/renderer/scena/camera completamente indipendenti da `state` — nessuna condivisione di geometria (costruisce da sé la `BufferGeometry` dal mesh-data ricevuto) né di renderer. L'unico punto di contatto con `viewport/core` resta l'API pubblica esistente: `register-frame-callback!`/`unregister-frame-callback!` (lo stesso aggancio per-frame che già usa il rescale del gizmo edit-attach), registrato **una volta sola** e condiviso da tutte le istanze — il callback itera l'atom `instances` (keyed) e sincronizza/renderizza ciascuna, copiando il quaternion della camera principale ogni frame senza toccarne posizione/target. `mount!`/`unmount!`/`set-content!` prendono ora una `key` scelta dal chiamante: `edit-mesh-split` usa la chiave `:context` per la sua Vista contesto singola (comportamento invariato — un'unica istanza montata resta pixel-identica alla vecchia posizione singleton), `mesh-board` monta una finestra per vista di confronto (`:intersection`/`:missing`/`:excess`), chiavi per-label così più confronti coesistenti in un programma restano gruppi di finestre distinti. Le istanze si impilano in una colonna (`.viewport-inset-column`, flex column-reverse) ancorata all'angolo in basso a destra del viewport. Nato per `edit-mesh-split`, deliberatamente tool-agnostico fin dall'origine — la generalizzazione N-istanze non ha richiesto di toccare `viewport/core`.

**Prima interattività** (brief-mesh-board-views.md Parte 4.3/4.4, feedback dal primo uso): l'inset resta non interattivo per rotazione/pan (l'orientamento sincronizzato con la principale è la feature), ma un'istanza si trascina dalla barra del titolo — al `mousedown` sull'header il container passa a `position: fixed` con `left`/`top` inline, uscendo dal flusso della colonna flex; `mousemove`/`mouseup` sono su `document` (non sul container) perché un trascinamento deve continuare a seguire il puntatore anche fuori dai 220×160px della finestrella — per questo `unmount!` li rimuove esplicitamente (altrimenti sarebbero listener orfani su `document`, che riferiscono un nodo DOM già staccato, ad ogni ciclo mount/unmount). La rotella sopra la finestrella zooma: un moltiplicatore `:zoom` per-istanza (clamp 0.2×-5×) applicato a `:dist` in `render-instance!`, con `preventDefault`/`stopPropagation` esplicite così l'evento non raggiunge mai le TrackballControls del viewport principale sottostante (l'inset è sovrapposto per z-index, non discendente del canvas principale — l'hit-testing del DOM già lo escluderebbe, ma gli handler lo confermano per iscritto). Posizione e zoom sono view state (`instances` atom), mai persistiti: un reload riparte dal layout di colonna di default.

**Il contenuto vuoto è una risposta, non un non-evento** (Parte 4.1): il primo bug segnalato dall'uso reale — quando due pezzi smettono di sovrapporsi, `:intersection` deve leggersi "vuoto", non continuare a mostrare l'ultimo risultato non vuoto. La causa in `mesh-board.cljs` era un `when-let` che saltava del tutto `set-content!` quando il booleano non produceva geometria utile; `update-compare-insets!` ora chiama sempre `set-content!`, con un'etichetta esplicita (`view-label`) quando il mesh è vuoto o `nil`. Ogni finestrella mostra anche il wireframe ghost del `reference` (ancoraggio spaziale, Parte 4.2) — e poiché `set-content!` inquadra sul bbox del ghost quando presente, la camera resta stabile sul riferimento mentre il candidato cambia ad ogni ritocco, invece di saltare ad ogni rivalutazione.
...
**Modi interattivi**:
- `session` in `editor/edit_attach.cljs` (ex `pilot-state` in `pilot_mode.cljs`). Mantiene la sessione edit-attach: marker nel sorgente, lista comandi (verbatim e gesti), stato di gizmo e pannello. Lo skip flag non è più per-modulo: vive nel layer `modal_evaluator` (`arm-skip!`/`consume-skip!`), armato solo dai cancel che lasciano il marker nel sorgente (edit-bezier, tweak permanente).
- lo stato di tweak in `editor/tweak_mode.cljs` (ex `test_mode.cljs`): slider attivi, ultimo valore di ciascuno, hook per evitare loop di re-eval.
- `interactive-mode` in `editor/state.cljs`: keyword che dice quale sessione modale è attiva (`:edit-attach`, `:tweak`, ...) o `:normal`.

Nessuno di questi atom viene resettato a Run. La sessione AI, le impostazioni LLM, lo stato di un'eventuale sessione modale in corso, sopravvivono al ciclo di valutazione: lo stato è dell'utente, non del codice utente.
...
`sci-ctx-ref` vive in `editor/state.cljs`. È un *mirror* di `sci-ctx`. Perché esistono due copie? Perché `editor/repl` importa `editor/tweak_mode` (per gestire l'integrazione del tweak con la valutazione), ma `editor/tweak_mode` ha bisogno del context SCI per espandere le forme tweak. Importare `editor/repl` da dentro `editor/tweak_mode` creerebbe una dipendenza circolare. La soluzione adottata è di replicare il riferimento al context in un atom che vive in `editor/state.cljs` (un modulo neutro, senza dipendenze verso repl o tweak_mode), e fare in modo che `repl/reset-ctx!` aggiorni anche `state/sci-ctx-ref`. Il tweak_mode legge da `state/sci-ctx-ref`, non da `repl/sci-ctx`.
...
| sessioni modali (edit-attach, tweak) | preservato | preservato | da exit di modo |
...
| Turtle, extrude/loft/revolve, shape-fn, warp | Identico | Identico |
...
Una famiglia di macro del DSL apre sessioni interattive che sospendono il flusso ordinario dell'eval, prendono il controllo di tastiera e viewport, e producono come effetto finale una riscrittura del sorgente nell'editor. Al 2026-07 i membri sono sette: `tweak` (slider live sui literal numerici), `edit-bezier` (editing di curve con punti di controllo), `edit-path` ed `edit-path-2d` (editing di percorsi per nodi draggabili, 3D e 2D), `edit-image-board` (calibrazione di immagini di riferimento), `edit-attach` (riposizionamento e deformazione di una mesh esistente, ex `pilot`), `edit-mesh-split` (decomposizione interattiva a piani di taglio, sezione 11.2.5).

I sei `edit-*` seguono una grammatica comune: `edit-X` accetta gli stessi argomenti di `X`, alla conferma si riscrive nella forma `X` con i valori risultanti, e la forma `X` è ri-editabile riapplicando il prefisso, un round-trip completo. Il gesto è supportato dalla voce "Edit" del menu contestuale dell'editor, che riscrive la testa della form sotto al cursore secondo una tabella (`path → edit-path`, `attach → edit-attach`, `mesh-split → edit-mesh-split`, ...) e rilancia l'eval. Il cancel (Esc) fa il percorso inverso: strippa il prefisso lasciando il body intatto, così l'annullamento non lascia sessioni armate nel sorgente. `tweak` è il membro anomalo della grammatica: avvolge un'espressione invece di prefissarla, e alla conferma si rimuove lasciando i valori scelti. Tutti sono pensati per il workflow di iterazione fine: l'utente non deve uscire dall'editor, calcolare un nuovo valore mentalmente, riscriverlo, rivalutare; la sessione interattiva chiude il giro.

Questi strumenti sono modal nel senso classico del termine: il programma entra in uno stato in cui l'input dell'utente non è più Clojure ma gesti specializzati (tasti, slider, drag nel viewport). La differenza rispetto a un'interazione UI ordinaria è che il modello di dati su cui agiscono è il sorgente, e l'effetto finale è una sua modifica. Il loro grosso vive in moduli paralleli sopra uno scheletro condiviso (`editor/modal_evaluator.cljs`, 371 righe, sezione 11.2.4): `tweak_mode.cljs`, `edit_bezier.cljs`, `edit_path.cljs`, `edit_image_board.cljs`, `edit_attach.cljs` (745 righe, più `gizmo.cljs`, 892), `edit_mesh_split.cljs`. Si appoggiano in modo intenso a CodeMirror, che è la prima cosa da raccontare.
...
Il modulo espone una superficie pubblica ampia, una trentina di funzioni divisibili in sette gruppi: lifecycle (`create-editor`, `focus`, `destroy`), accesso al contenuto (`get-value`, `replace-range`, `insert-at-cursor`), gestione di cursore e selezione, navigazione strutturale bracket-aware sui form (`get-form-at-cursor`, `get-next-form`, `parse-form-elements`), manipolazione di form testuali (`replace-form-element`, `slurp-form`), gestione dell'AI focus (`set-ai-focus!`, `update-ai-focus!`), gestione dei blocchi AI (`find-ai-block`, `replace-ai-block`, `promote-ai-block-to-step!`), utility UI (toggle line numbers, scroll-to-line, undo/redo programmatico). I consumer principali: `core.cljs` per il lifecycle e i bottoni della UI principale, `ai/auto_session.cljs` e `ai/describe.cljs` per scrivere e leggere blocchi commentati nel sorgente, e la famiglia dei modal evaluator (via il layer `modal_evaluator` e direttamente) per il replace dei form. La superficie larga è il riflesso del fatto che codemirror.cljs non è solo un editor: è la superficie di edit condivisa dei sottosistemi che riscrivono il sorgente.
...
L'apertura della sessione avviene durante un eval ordinario. La macro espande a una chiamata che produce comunque la mesh attesa, quindi il viewport mostra il risultato come per qualunque altra eval; in parallelo, il modulo `tweak_mode` cammina l'espressione e indicizza i suoi literal numerici, costruendo un pannello di slider sotto al viewport. Il matching fra slider e literal è posizionale: l'n-esimo slider corrisponde all'n-esimo numero incontrato camminando l'albero.
...
#### 11.2.3 Edit-attach: manipolazione diretta che riscrive il sorgente

La macro `(edit-attach mesh cmds...)` (fino al 2026-07-08 `pilot`, che sopravvive come alias) apre una sessione in cui l'utente riposiziona e deforma una mesh esistente; alla conferma riscrive la form in un `(attach mesh cmds...)` flat che contiene la sequenza di comandi accumulata, compattando i consecutivi fondibili. La forma accetta anche comandi preesistenti: riaprendo un `attach` (tipicamente dal menu Edit) i suoi comandi entrano in sessione come item verbatim, testo sorgente opaco preservato carattere per carattere, e i nuovi gesti si accodano. Il round-trip senza gesti è l'identità sul sorgente.

L'input ha due layer. Il gizmo (`editor/gizmo.cljs`) è un manipolatore combinato disegnato sulla creation-pose della mesh: tre frecce di traslazione e tre anelli di rotazione orientati sugli assi della pose, che mappano uno-a-uno sui comandi turtle (`f`/`rt`/`u`, `th`/`tv`/`tr`), più tre maniglie di stretch. Ogni gesto concluso emette un comando nella lista di sessione; il gizmo è letteralmente il frame della turtle reso visibile, e l'utente che dragga impara il DSL guardando i comandi apparire. La tastiera resta come layer di precisione: le frecce emettono passi quantizzati sugli stessi parametri che fanno da griglia di snap ai drag. Un toggle oggetto/origin cambia la semantica dei gesti: in modalità origin gli stessi handle emettono la famiglia `cp-*` (l'ancora si sposta relativamente alla geometria), e un click su un vertice della mesh aggancia la pose lì, emettendo le `cp` corrispondenti.

Le maniglie di stretch sono disegnate sugli assi materiali della mesh, non su quelli della pose: dal 2026-07-08 le direzioni di stretch sono incollate al corpo (il frame di creazione trasportato dalle sole operazioni rigide, tracciato nel replay come offset locale aggiornato dalle sole rotazioni `cp`), mentre il pivot resta la posizione dell'ancora. Le due terne divergono visibilmente solo quando la storia contiene rotazioni `cp`, ed è un'onestà visiva voluta. Il principio: traslazioni e rotazioni sono operazioni di piazzamento e vivono nel frame dell'ancora, lo stretch è una deformazione e appartiene al corpo. L'invariante che ne discende: `stretch-f` prima e dopo una `cp-th` produce la stessa mesh.

L'ingresso in sessione è in due fasi, e questa è la cosa architetturalmente interessante della sezione. La macro chiama `request!`, che valida l'argomento, individua la form nel sorgente con un matching a bilanciamento di parentesi, prende il mutex e prepara lo stato; ma non installa subito gli handler. La ragione è che l'`edit-attach` è dentro un eval in corso, e il valore che la macro deve restituire all'eval è la mesh (con i comandi preesistenti applicati), non un effetto interattivo. Solo dopo che l'eval completa e il driver del layer modale controlla la flag `requested?`, il modulo entra davvero con `enter!`: installa keydown e pointer handler, monta gizmo e turtle indicator, costruisce il pannello. La separazione `request!` / `enter!` è il riflesso del fatto che in Ridley l'eval è il padrone del flusso: la sessione non lo interrompe, attende che finisca.

Per il resto, edit-attach condivide con la famiglia la distinzione fra script mode e REPL mode con la stessa semantica (il modello vs l'esplorazione effimera), il mutex centrale, e la riscrittura finale via layer modale. Le sue specificità di superficie (il keymap, i valori di snap, la compattazione, i dettagli visivi del gizmo) sono materia per la guida utente.
...
> **Aggiornamento (2026-06-08): lo scheletro ora c'è.** All'arrivo del terzo modal evaluator (`edit-bezier`) il debito di 15.2.1 è stato pagato: `editor/modal_evaluator.cljs` raccoglie le meccaniche condivise (mutex, skip flag, mount/unmount del pannello, keyhandler con fase configurabile, re-eval script, commit `cm/replace-range`+`run-definitions`, splice e ricerca della forma nel sorgente) e il driver a due fasi come registro generico di kind. Tweak e pilot delegano a questo layer; `core.cljs` non nomina più i moduli concreti ma chiama `modal/requested?` / `modal/enter!` / `modal/active?` / `modal/cancel-active!`. Pilot ed edit-bezier sono deferred (enter! dal driver post-eval); tweak è il caso degenere sincrono (apre dentro `start!`, registra `requested? → false`). Vedi 15.2.1 per la nota sul costo realmente recuperato.

> **Aggiornamento (2026-07-09): la famiglia a sei e la fine del trucco dello skip.** Sopra lo scheletro sono arrivati `edit-path`, `edit-path-2d`, `edit-image-board` e il rename `pilot` → `edit-attach` (con gizmo di viewport), con la grammatica `edit-X ↔ X` e la voce di menu "Edit" descritte nell'intro di 11.2. Lo skip flag, nato per evitare che la macro rientrasse in sessione sulla re-eval triggerata dal modulo stesso, ha perso la maggior parte dei suoi consumatori: il cancel di edit-path, edit-path-2d, edit-image-board ed edit-attach ora strippa il prefisso dal sorgente, quindi la re-eval non incontra più il marker e non c'è nulla da saltare. Lo armano solo i cancel che lasciano il marker nel sorgente (edit-bezier, e tweak nella variante permanente). Un default insidioso (`arm-skip?` che valeva `true` se omesso in `reeval-script!`) ha prodotto un bug di rientranza deterministico, diagnosticato e risolto con brief-edit-attach-reentry (dev-docs): l'argomento è ora obbligatorio, e il ramo di skip logga un warning se consuma un flag inatteso.

#### 11.2.5 Edit-mesh-split: gizmo condiviso senza stretch, undo a stack, sintesi del delta

Il settimo modal evaluator (`edit-mesh-split`, 2026-07-11, dev-docs/brief-edit-mesh-split.md) segue la grammatica `edit-X ↔ X` e lo scheletro condiviso come gli altri, ma diverge in tre punti architetturalmente notevoli.

**Nessuna seconda rappresentazione della posa, ma il gizmo è condiviso.** Il piano di taglio *è* la posa della turtle: `position` + `heading` definiscono il piano, `up` orienta il quad renderizzato — non c'è una seconda rappresentazione della posa da tenere sincronizzata. Il primo brief aveva escluso i drag handle dal proprio scope leggendo la famiglia solo su `pilot`/`edit-bezier`, senza contare che `edit-attach` esisteva già con un gizmo pienamente disaccoppiato dalla propria sessione (dev-docs/addendum-brief-edit-mesh-split.md, 2026-07-11): `editor/gizmo.cljs` dipende solo da una posa, opzioni e callback di gesto, quindi il riuso è stato diretto, non un'estrazione. Le estensioni sono tutte additive (default = comportamento di `edit-attach` invariato, che non passa nessuna di queste chiavi): un'opzione `:handles` che filtra quali handle vengono costruiti (`edit-mesh-split` chiede solo `#{:translate :rotate}`, niente stretch — un piano di taglio non ha scala); `:nudge-mesh? false` che sopprime il nudge automatico di TUTTA la preview che `:mode :object` farebbe di default; e tre callback opzionali — `:on-drag-start`/`:on-drag-end`, e `:on-drag` che fornisce ad ogni pointermove l'esatto `{:cmd-type :value}` che `on-commit` emetterebbe se il drag finisse in quell'istante (stessa selezione mode-dependent — plain vs `cp-*` — che `on-pointer-up` già usa).

**Solo il piano segue il drag ad ogni frame, i pezzi si ricalcolano dal vivo ma con un throttle adattivo.** Tre iterazioni, tutte da feedback dal vivo di Vincenzo il 2026-07-12. (1) Con `:mode :object` di default, `viewport/nudge-preview!` faceva seguire l'INTERA preview al drag — quad, cono, E i pezzi — e si leggeva come "la mesh si muove", confuso, perché concettualmente solo il PIANO si sta muovendo: fix, `:nudge-mesh? false` sopprime quel nudge globale. (2) Il ricalcolo vero (split/`convex?`/volume) inizialmente avveniva solo `on-commit`, al rilascio, con la preview intera portata a un grigio neutro per la durata del drag (mai un colore stantio, ma nemmeno mai un colore aggiornato). Vincenzo ha chiesto l'opposto: colori aggiornati DURANTE il drag, non solo dopo. La split-by-plane misurata dal vivo varia enormemente per costo (~12ms su un box di 4600 tri, 50-200ms su una mesh densa di 8000 tri a seconda del taglio) — troppo per ricalcolare ad ogni pointermove senza rischiare di bloccare il thread principale a ripetizione. Soluzione: `recompute!` misura il proprio costo (`js/performance.now`) e lo salva in sessione; `on-gizmo-drag!` lo consulta per decidere se il ricalcolo è dovuto (subito al primo tick di un drag, poi throttle adattivo = 2× l'ultimo costo misurato, floor 50ms) — una mesh economica si aggiorna quasi ad ogni frame, una costosa rallenta invece di accodare ricalcoli bloccanti uno sull'altro. Il piano (quad+cono) segue comunque ad OGNI tick indipendentemente dal throttle: `viewport/replace-preview-at!` (nuova, fattorizza `build-preview-object` da `show-preview!`) ricostruisce solo le posizioni 0/1 della preview — geometria minuscola e fissa (4 e ~11 vertici), quindi economica indipendentemente dalla dimensione della mesh tagliata — lasciando intatto tutto il resto. La posa provvisoria del drag vive in `:preview-pose` (sessione), mai scritta su `:live-pose` finché non arriva un vero commit — `working-pose` (nuovo, usato sia da `render!` sia da `recompute!`) risolve l'uno o l'altro. (3) I pezzi consumati e le etichette sono descritti sotto, in "Leggibilità della scena". Verificato via `PointerEvent` sintetici sul canvas ad ogni iterazione: prima del fix-1, `matrixAutoUpdate` di TUTTA la preview passava a `false` durante il drag (tutto si muoveva); dopo, solo quad/cono. Il costo di `recompute!` è stato misurato direttamente (non stimato) sulle due mesh di cui sopra per calibrare il floor/moltiplicatore del throttle.

**Conferma a singola azione.** Ogni altro `edit-*` ha un solo livello di conferma (`Enter` = commit). `edit-mesh-split` ne avrebbe bisogno di due — accetta questo taglio e continua, oppure chiudi la sessione — ma la soluzione adottata (decisa conversazionalmente con Vincenzo, non nel brief originale) collassa i due in una sola azione: `Enter` significa sempre "accetta `:behind` come pezzo definitivo", e l'effetto (continua vs. chiude) è una funzione dello stato geometrico del piano (quale metà è vuota), non del tasto premuto. Il piano diventa grigio (taglio no-op, `Enter` disabilitato) quando `:behind` è vuoto, e assume un colore dedicato (piazzamento terminale, `Enter` chiude) quando `:ahead` è vuoto — due stati degeneri distinti da un fatto della posa, non da una nozione di direzione. `Ctrl`/`Cmd`+`Enter` è zucchero puro (teletrasporta al piazzamento terminale poi applica la stessa logica) — nessun percorso di emissione separato.

**Sintesi del delta invece di replay del transcript.** Come `edit-bezier` (punti di controllo, non cronologia mouse), l'emissione di `edit-mesh-split` non è un replay dei tasti: per ogni taglio accettato sintetizza il delta canonico minimo `(th)(tv)(tr)(f)(rt)(u)` fra la posa del taglio precedente e quella corrente. La funzione (`turtle/synthesize-delta`, pura, testata a proprietà per round-trip) riusa la stessa decomposizione th-poi-tv-più-roll-residuo che `lower-commands` già usa per abbassare bezier/arc in sorgente leggibile (`bezier/compute-rotation-angles`, `bezier/residual-roll-deg`) — non è stata inventata nuova matematica. Un dettaglio non ovvio scoperto implementando: dato che rotazione e posizione sono gradi di libertà indipendenti, orientarsi verso la DIREZIONE della posizione target (anziché verso l'HEADING target) e sperare che coincidano funziona solo nel caso degenere in cui la posizione target è "dritta avanti" rispetto all'orientamento finale — il caso generale richiede sempre i tre angoli di rotazione ancorati all'heading target, poi la traslazione proiettata sugli assi del frame finale (`f`/`rt`/`u`), non solo `f`. Verificato con un test di proprietà che include pose la cui differenza non è raggiungibile con un singolo `f`.

Il re-entry non replica il pattern di `edit-attach` (comandi verbatim preservati carattere per carattere) né quello di `edit-path` (replay dei comandi alti attraverso le primitive turtle reali per ricostruire i nodi): riusa direttamente la struttura annidata che la composita `mesh-split` (sezione 7, Spec) già produce — lo stack dei tagli accettati si ricostruisce percorrendo il risultato composito valutato una volta, senza replay del sorgente. Il costo è che l'emissione è sempre canonica: ri-aprire una sessione e richiuderla subito riproduce lo stesso output byte per byte, mai il testo originale se differiva (es. per formattazione).

**Leggibilità della scena (addendum 2026-07-11, rifinito 2026-07-12).** Una variabile visiva per dimensione semantica: la tinta (verde/rosso) resta convessità delle due metà live; la solidità (opacità 0.88 vs 0.18) distingue `:behind` (pieno, ciò che `Enter` accetta) da `:ahead` (lavato); lo STILE (non più tinta+alpha) marca i pezzi già consumati. Prima iterazione: grigio a bassa opacità, con etichetta billboard (`viewport/set-labels!`) col nome che avrà nella forma emessa. Feedback dal vivo (Vincenzo, 2026-07-12): la mesh a bassa opacità leggeva "blu" sotto l'illuminazione della scena e si confondeva sia col quad del piano sia col `:behind` corrente; le etichette billboard aggiungevano solo confusione. Fix, entrambi presi dalle alternative già previste dall'addendum: i pezzi consumati sono ora un **wireframe** (`:type :wireframe`, nuovo item di `show-preview!`, stessa tecnica EdgesGeometry di `show-wireframe-preview!` ma componibile e a colore configurabile) — uno STILE strutturalmente diverso da qualunque mesh piena, illeggibile-proof rispetto a illuminazione/tinta; le etichette sono state rimosse (nessuna sostituzione per ora — un'indicazione fissa e dimensionata al pezzo è un'idea aperta, non implementata). Il cono di orientamento punta lungo `+heading` (verso `:ahead`, come il naso della turtle stessa) invece che verso `:behind`: la lettura voluta è "il cono mostra dove si continua, `Enter` prende il lato opposto", stessa convenzione di `extrude`. Verificando dal vivo è emerso un bug preesistente indipendente dall'addendum: `orientation-cone` passava `:color`/`:opacity` in cima alla mappa della mesh, ma `create-three-mesh` (viewport/core.cljs) legge solo `(:material mesh-data)` — il cono ha sempre renderizzato col colore di default (`0x00aaff`) invece che col colore del piano, dalla prima consegna. Fix: annidare sotto `:material`, come già fanno `half-preview-item`/`ghost-item`.

**Da ghigliottina ad albero (Tranche 2, 2026-07-12, dev-docs/brief-mesh-components-tree.md).** Il primo uso reale sul mount multiboard ha mostrato che il modello lineare non basta: dopo pochi tagli sia `:behind` sia `:ahead` sono concavi (accertamento fase 2). Il refactor stack→albero, delimitato come previsto (~7 funzioni, layer gesti e rendering invariati), estrae la logica strutturale PURA in `ridley.editor.mesh-split-tree` (ids, origini, log cronologico dei gesti, selezione pezzo corrente, naming, emissione — nessun WASM/DOM, 16 test in node). La sessione tiene `:tree` (la struttura pura) + `:piece-meshes {id {:mesh :manifold}}` (side-table WASM). Cambiamenti chiave: il `:remaining` singolo diventa una collezione di pezzi aperti + selettore (click via `viewport/raycast-preview-pick` sui preview taggati `:pick-id`, o tasto `n`); `:accepted`, che faceva tre lavori (stack undo, sorgente emissione, derivazione remaining), è sdoppiato in struttura-pezzi vs log-gesti (undo = pop dell'ultimo gesto strutturale, cross-branch, cronologico). Nuovo gesto **separa componenti** (tasto `s`): materializza le componenti connesse (`mesh-components`, Parte 1) come pezzi distinti — a costo topologico, senza piano. **Criterio di finitezza per-componente** (Parte 2): verde = ogni componente convessa (una U coi due rebbi convessi è *già finita*, va separata non tagliata). **Keep-alive del Manifold**: ogni pezzo aperto tiene vivo il suo oggetto Manifold, convertito una volta (`manifold/split-live`, che riusa il Manifold e ritorna anche i volumi delle metà) — il ricalcolo per keystroke smette di pagare la conversione mesh→manifold (~8ms, accertamento B2); `.delete` garantito su undo/cancel/commit via `cleanup!` (punto unico, nessun leak). **Emissione ad albero**: un let a catena di composite lineari auto-contenute (una `mesh-split` per ramo, ciascuna col suo path e i suoi mark path-scoped) più una destrutturazione `mesh-components` per separazione; la topologia dell'albero vive in quale binding alimenta quale chiamata. **Conseguenza**: il re-entry non richiede alcun meccanismo nuovo — ogni chiamata emessa è una composita lineare come prima, la si riapre cambiando quella singola `mesh-split` in `edit-mesh-split` (macro invariato), e il tree si ricostruisce dal composito valutato. La conferma cambia di conseguenza: `Enter` taglia il pezzo corrente, o committa quando ogni pezzo è finito ("tutto verde" = tutti i pezzi dell'albero finiti); `Ctrl/Cmd+Enter` committa comunque. `split-parts` (`manifold/core.cljs`) era già DFS behind-before-ahead → generalizza gratis.

**Focus e navigazione esplicita (addendum, dev-docs/addendum-brief-mesh-components-tree.md).** Il primo uso reale ha mostrato una collisione di gesti: da quando il mouse manipola il piano (handle), il click-per-selezionare-pezzi dava due significati allo stesso gesto → selezioni accidentali. Rimedio: **un canale un mestiere**. Il click-select è rimosso (il mouse fa solo il piano); la navigazione fra pezzi aperti è **esplicita e deterministica** — tasti `n`/`p` (next/prev, DFS sui soli aperti) e bottoni ◀/▶ nel pannello. La presentazione diventa **focalizzata sull'operazione**: il pezzo corrente è pieno col semaforo, gli altri aperti arretrano a contesto tenue (alpha bassa, nessuna tinta di convessità — il semaforo appartiene al solo corrente), i finiti restano wireframe; un **toggle di rivelazione** (`r`, view-only, fuori dal log) riporta temporaneamente tutti a piena visibilità. Ancoraggio testuale: una riga di posizione nel pannello (`piece <name> (2/5) · open · 1 component`) e le etichette in scena mostrano lo **stesso nome** dell'emissione. `mesh-split-tree` guadagna `cycle-current` con direzione e `position-info`; nessun cambiamento a taglio/undo/finitezza/emissione — solo navigazione e presentazione (round-trip d'emissione invariato). **Addendum 2** (etichette non invasive): i billboard camera-facing occludevano la vista → di default **nessuna etichetta in scena**; le etichette compaiono solo col toggle di rivelazione, **world-anchored** (orientamento fisso, mai camera-facing) e occluse dalla geometria (`viewport/set-world-labels!`/`clear-world-labels!`, in un atom separato che `update-panels-billboard` non tocca — distinto dal `set-labels!` billboard usato da edit-path/edit-image-board). Il contratto dei nomi (scena↔pannello↔emissione) resta invariato: cambia solo quando/come si mostrano.

**Integrazione simmetria (brief-mesh-symmetry Parti 4-5).** Sui primitivi DSL `mesh-mirror`/`mirror?`/`symmetry-planes` (Parti 1-3, `manifold/core` + ns puro `ridley.geometry.symmetry`), la sessione aggiunge: (Part 4) gesto **`y` proponi piano** — `symmetry-planes` del pezzo corrente (cache per pezzo, immutabile), teletrasporto al primo candidato, pressioni successive ciclano; il calcolo (~250-450ms) è dietro uno stato *pending* visibile (setTimeout per far rendere il pending prima di bloccare). **Badge specchio**: il gate volumetrico è gratis per-keystroke (già dai volumi dello split); se passa e il piano resta fermo per un **debounce di 300ms**, la conferma B6 (`symmetric-about-plane?`, 77-148ms) parte in background e il pannello mostra "behind = ahead reflected"; ogni movimento del piano (nuovo `recompute!`) azzera badge+timer — i colori non mentono. **Annotazione emissione**: un `:cut` con `:mirror?` confermato porta un commento `;; :cut-N: piano di simmetria` sulla riga del suo mark (mtree `run-binding`; i commenti non toccano l'eval → round-trip invariato). (Part 5) gesto **`d` decomponi a specchio**: se il gemello del pezzo corrente (l'altra metà di un taglio-specchio confermato) è decomposto, `mtree/subtree-gestures` estrae i suoi gesti e li si **rigioca** sul pezzo corrente con pose **riflesse** (`mtree/reflect-pose` — solo position+heading, l'up libero via `free-up`; la riflessione preserva i prodotti scalari quindi `:behind`→`:behind` senza rimappatura). Tutti i gesti del replay condividono un `:group`; l'undo group-aware (`mtree/undo`) li rimuove atomicamente — un solo gesto strutturale. Emissione ordinaria (chiamate `mesh-split` verso le pose riflesse). Parti 4-5 **live-verificate 2026-07-13** (flusso specchio completo su una U reale).

**UI di sessione + livello di simmetria (addendum-3-session-ui.md, 2026-07-13).** L'uso reale ha falsificato la presentazione precedente: (A) ogni gesto ha ora un **bottone** con stato abilitato/disabilitato + ragione (tooltip), da un'unica `gesture-availability`; e **nessun no-op silenzioso** — un tasto che non può agire scrive la stessa ragione in una riga di stato del pannello (prima il feedback finiva in `capture-println` → console REPL, invisibile durante il modale: era la vera causa per cui `y`/`d` sembravano inerti). (B) **Due stati di scena netti**: lavoro (solo il pezzo corrente + piano; il composito base valutato è nascosto su enter via `viewport/hide-user-geometry!`, PRIMA di gizmo/preview così restano visibili — il preview del modale è l'unica verità di scena) e reveal (tutti i pezzi). (C) etichette **billboard** solo in reveal. Triage `y`/`d`: nessun bug di binding — un **bug vero** nel solutore agli autovalori `jacobi-eigen-3x3` (segno dell'angolo di rotazione invertito → assi principali spazzatura su qualsiasi covarianza non diagonale) impediva a `symmetry-planes` di trovare piani su pezzi CAD allineati; corretto, `candidate-planes` prova ora *sempre* gli assi del bbox. Il **livello di simmetria** è mostrato ("quasi simmetrico (scarto ~X%)" vs "esatta") perché la tolleranza al 2% accetta pezzi simmetrici tranne una piccola feature (es. un thread interno): `symmetry-planes` porta `:symmetry-ratio`, nuova `mirror-difference-ratio` espone il numero, e il badge + l'annuncio di `y` lo mostrano.
...
**Emissione nuda + `split-tree` (brief-split-tree.md, 2026-07-15).** Il let-chain era la forma emessa in *ogni* caso, e per il caso comune — una sola run lineare, nessuna separazione — non era solo rumore: **rompeva il round-trip**. La conferma riscrive la *sola forma marcata* (`modal/replace-source!` su `[from,to]`), quindi riaprire la `mesh-split` dentro il let e richiudere annidava un `let` fresco nella posizione di binding del let esterno — sorgente rotto, non solo verboso. Il caso lineare emette ora la **chiamata nuda** (`(mesh-split mount (path …) [:cut-1])`): niente let, niente destructuring — chi vuole i nomi se li scrive, o chiama `split-tree`. La nuda *è* letteralmente l'arità a 3 del macro `edit-mesh-split`, quindi il re-entry ("rimetti `edit-` davanti") non richiede alcun meccanismo: era già la promessa dell'architettura ad albero (ogni chiamata emessa è una composita lineare auto-contenuta), ma la forma emessa la contraddiceva. Al momento (caso lineare) run multiple restavano un `let`-chain — vedi il paragrafo seguente per come questo limite è caduto. **Conseguenza sul valore**: la nuda ritorna il composito grezzo `{:behind :ahead}`, non più la mappa nome→mesh. `(split-tree result)` (accanto a `split-parts` in `manifold/core`, puro) è il ponte: stesse foglie, stesso ordine, nominate `{:piece-N …}` — e i nomi coincidono con quelli che `name-map` dà alle etichette in scena, perché la nuda esiste solo per la run singola, che è esattamente il caso in cui gli `:ahead` intermedi non esistono e i due ordinamenti collimano. **Guard**: `mesh-board` e `attach` rifiutano il composito grezzo nominando `split-tree`. Il predicato è `manifold/split-composite?` (chiavi `:behind`+`:ahead`), e la regola è uniforme *apposta* — a un taglio solo entrambi i valori sono mesh, quindi `meshes-collection?`/`leaves-of` lo accettavano **in silenzio** come albero di due pezzi chiamati `:behind`/`:ahead`: un successo che insegna la forma sbagliata e si rompe al secondo taglio (dove l'`:ahead` annidato arriva *come* mesh agli scaffold). La conversione resta compito di `split-tree`, esplicita nel sorgente dell'utente, mai una coercizione implicita. **Commenti di simmetria**: restano fuori dalla chiamata (dentro sarebbero argomenti per `cm/parse-form-elements`). Il re-entry ricostruisce l'albero dalla *geometria* del composito e non recupera `:mirror?`, quindi il tool li scrive una volta e poi appartengono al sorgente — sopravvivono al giro proprio perché fuori dallo span riscritto, al prezzo di poter invecchiare se il taglio riaperto viene spostato.

**`mesh-split` ad albero + emit sempre nudo, SEPARATE fuori dall'editor (brief-split-tree.md, 2026-07-15, seguito).** Il limite lasciato aperto sopra — run multiple → `let`-chain — cade: il terzo argomento di `mesh-split` si generalizza da vettore piatto `[mark …]` a **mappa ricorsiva** `{mark sub-spec …}`, dove `sub-spec` è `nil` (il mark taglia e basta), un `path` nudo (il pezzo staccato a quel mark viene ritagliato a tutti i marchi di quel path, in ORDINE di path — mai l'ordine delle chiavi della mappa, che Clojure non garantisce), o `[path spec]` (forma piena, rami nei rami). Il frame dei sotto-path è la **cut-pose del mark**, non la turtle live: il pezzo ritagliato non ha più nulla a che fare con dove la turtle è finita nel frattempo. Implementazione condivisa fra taglio vero e re-entry: `impl/resolve-mesh-split-plan` (puro, ricorsivo) risolve `(entry-pose, path, spec)` in un piano ordinato `[{:mark :pose :sub} …]`; `implicit-mesh-split` lo esegue tagliando davvero (`split-plan`, sostituisce il vecchio `guillotine-split`); `edit-mesh-split`'s `build-reentry-tree`/`walk-reentry` lo cammina in parallelo al composito già valutato, ricorrendo in `:sub` per ricostruire un ramo esattamente come fu tagliato. **`mesh_split_tree.cljs`'s `emit`** rispecchia la stessa ricorsione lato emissione (`level-text`): per ogni run, se nessun taglio ha un `:behind` che inizia un'altra run → vettore `[:cut-1 …]` invariato; altrimenti mappa, con ogni sotto-livello annidato (`(path …)` nudo se non ramifica oltre, `[(path …) {…}]` se ramifica ancora) — **sempre una sola chiamata**, mai più. I commenti `;; piano di simmetria` restano locali al loro run e finiscono dentro la mappa, prima della prossima entry o della `}` di chiusura (stesso trucco newline-doppia di prima, generalizzato). **Parte 3 — SEPARATE esce dall'editor**: il gesto `s`/`mtree/separate`/`separation-binding` sono rimossi; `mesh-components` resta DSL normale, applicata a mano sul risultato della chiamata nuda (`(mesh-components (:behind AA))`, o su una componente riaperta con `(edit-mesh-split (nth (mesh-components …) 0))`). Nessuna nuova notazione: componibile con quello che già c'era.
...
Una nota terminologica utile per chi naviga il codice. Il backend del macro `tweak` (live sliders sui literal numerici, sezione 11.2.2) vive in `editor/tweak_mode.cljs`. Fino al 2026-06-08 il file si chiamava `test_mode.cljs` — un residuo dei primi spike in cui tweak era denominato "test mode" — e collideva con la cartella `test/`, confondendo sistematicamente chi cercava i test reali del progetto (che vivono solo in `test/`). La rinomina, micro-voce di debito di 15.2.1, è stata eseguita insieme all'estrazione del modal evaluator.
...
A questo livello si aggiunge una validazione applicativa specifica del confine macro DSL: `validate-extrude-path!` in `editor/impl.cljs` rifiuta i comandi `:inset`, `:scale`, `:move-to` quando finiscono in `extrude`, `extrude-closed`, `loft`, `loft-n` (sono comandi che hanno senso solo in modalità path libero). Ha quindici chiamanti dentro lo stesso file, e tre dei test in `repl_test.cljs` colpiscono direttamente questa validazione, che è quindi l'unica zona di asserzione di schema con copertura di test diretta. Non è un caso: la validazione esiste perché un utente che scrive DSL può sbagliare in modo prevedibile, e i test verificano che gli errori prevedibili siano effettivamente intercettati.
...
**Runtime DSL**: ~775 righe di `repl_test.cljs` via harness SCI, copertura ampia del livello macro. Non coperta l'interazione dei modal evaluator (`edit_attach.cljs`, `tweak_mode.cljs`), né `text_ops.cljs` e l'integrazione CodeMirror; hanno invece test dedicati la matematica di proiezione del gizmo (`gizmo_test.cljs`), la semantica dello stretch nel frame materiale (`stretch_material_frame_test.cljs`) e i bounds SDF (`auto_bounds_test.cljs`).
...
> **Risolto (2026-06-08).** Il layer `editor/modal_evaluator.cljs` è stato estratto e tweak/pilot vi sono stati migrati; il modulo `test_mode.cljs` è stato rinominato `tweak_mode.cljs`. Il driver a due fasi (un tempo cablato su `pilot-mode` in `core.cljs`) ora vive nel layer come registro generico di kind, e tweak è modellato come il caso degenere sincrono. Il terzo modal evaluator, `edit-bezier`, è costruito sopra il layer (non clonando pilot). Nota sul costo realmente recuperato: lo scheletro genuinamente duplicato era ~80 righe di meccaniche (mutex, skip flag, pannello, keyhandler, re-eval script, commit, splice/parse del sorgente), non le ~500 stimate qui sotto — la mole dei due moduli è in larga parte logica specifica (la slider UI di tweak, il keymap di pilot) che non si condivide. I moduli non sono quindi scesi a ~150 righe; il valore pagato è il single-sourcing delle meccaniche e del driver, che è ciò che permette a `edit-bezier` di non essere un terzo clone. Il testo originale del debito resta sotto come registrazione storica.

Tweak (`editor/tweak_mode.cljs`, ex `test_mode.cljs`, 770 righe) e pilot (`editor/pilot_mode.cljs`, 579 righe) sono due implementazioni dello stesso pattern senza astrazione condivisa. Entrambi sono macro DSL che aprono una sessione modal interattiva, riscrivono il sorgente alla conferma, e si appoggiano allo stesso scheletro: stato in `defonce` atom locale, flag `skip-next-*` per evitare il rientro della macro, mutex centrale `claim-interactive-mode!` di `editor/state.cljs` per impedire sessioni concorrenti, pannello DOM inserito nello stesso punto del layout, keyhandler globale in capture phase, distinzione fra script mode e REPL mode, costruzione testuale del replacement, riscrittura via `cm/replace-range` seguita da `run-definitions-fn`.
