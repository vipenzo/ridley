...
> **Nota (Fase 3, dev-docs/brief-recording-highlevel-fase3.md, 2026-07-07)**: `:pure`/`:span` e `:veer-deg` — i tag di **ricostruzione**, letti solo dall'editor 2D (il primo) e dal vecchio guard rail-start basato su micro (il secondo) — sono stati rimossi dall'emissione insieme ai loro ultimi lettori: sia il lato 3D (`seed->nodes-3d`, Fase 2a) sia il lato 2D (`seed->nodes`/`project-2d-to-xy`, dev-docs/brief-recording-highlevel-lettura-2d.md) di edit-path interpretano i comandi alto livello del path (`:commands`) direttamente, e il guard rail-start (`validate-rail-start!`, Fase 2b) calcola l'angolo di veer analiticamente (`curve-entry-veer-deg`, extrusion.cljs). Zero occorrenze di entrambi in `src/` (grep di uscita). `:smooth`/`:arc-cap`/`:bez-cap` restano: sono il protocollo con cui `lower-commands` parla al suo unico interlocutore vivo, l'estrusione (`corner-rotation?`/`split-leading-cap`) — non riscontruzione per un lettore esterno, ma annotazione interna alla stessa cucitura tessellazione→estrusione.
