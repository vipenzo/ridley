...
Il path e' **comandi, non campioni**. I waypoint con posizioni e tangenti vengono calcolati piu' tardi, quando un consumer interpreta i comandi su una turtle. La funzione `walk-path-poses` (in `src/ridley/turtle/loft.cljs`) fa questo lavoro: replaya i comandi su una turtle locale, campiona la posa lungo il percorso, restituisce una sequenza di pose. Il sample rate non vive nel path, vive nel consumer.
