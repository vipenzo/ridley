...
| `:pure {:cmd :c1 :c2 :end :span}` | Il primo comando di un run bezier riceve la curva analitica risolta (control point e endpoint a record time) e lo span in comandi del run | `group-curve-runs`/`seed->nodes` (proiezione 2D di edit-path) | L'editor 2D ricostruisce il run tessellato come un singolo nodo bezier editabile. Rider ignorato da ogni altro consumer |

> **Nota (Fase 2a, dev-docs/brief-recording-highlevel-fase2a.md)**: il lato 3D di edit-path (`seed->nodes-3d`) non legge più `:pure`/`:span` — interpreta direttamente i comandi alto livello del path (`:commands`), senza passare da `path-micro-commands`. **Lettore residuo**: la proiezione 2D (`project-2d-to-xy`/`seed->nodes`) legge ancora `:pure`/`:span` via `path-micro-commands`; migrazione tracciata in un brief dedicato, prerequisito della rimozione in Fase 3.
