...
| `:arc-cap :lead` / `:arc-cap :trail` | I mezzi passi di rotazione con cui `arc-h`/`arc-v` aprono e chiudono l'integrazione midpoint (`rec-th-cap*`/`rec-tv-cap*`) | `split-leading-cap` | Il cap iniziale/finale dell'estrusione resta perpendicolare all'heading di ingresso/uscita. Dopo la Fase 2b il guard rail-start (`validate-rail-start!`) non legge più questo tag: un arco alto livello (`:arc-h`/`:arc-v`) è esente per costruzione (il suo vero tangente È l'heading in ingresso), quindi non richiede alcun calcolo |
| `:bez-cap :lead` | Le rotazioni prima del primo `f` di un run bezier (`rec-th-smooth-cap*`, `rec-tv-smooth-cap*`, `rec-tr-smooth-cap*`) | `split-leading-cap` | La finestra iniziale del run è esclusa dalla misura a corda del cap; il cap dell'estrusione parte perpendicolare all'heading pre-finestra |
| `:veer-deg` | Insieme a `:bez-cap :lead` sui `th`/`tv` del lead (non sul `tr`, il cui limite continuo è zero per costruzione) | — (senza lettori dal 2026-07, rimozione in Fase 3) | Era l'angolo analitico tra l'heading d'ingresso e c1−p0, letto dal vecchio guard rail-start basato su micro. Dopo la Fase 2b (`dev-docs/brief-recording-highlevel-fase2b.md`) `validate-rail-start!` calcola lo stesso angolo direttamente dalle componenti locali di `:c1` sul comando alto livello (`analytic-veer-deg`/`curve-entry-veer-deg`, extrusion.cljs), senza bisogno del tag |
| `:pure {:cmd :c1 :c2 :end :span}` | Il primo comando di un run bezier riceve la curva analitica risolta (control point e endpoint a record time) e lo span in comandi del run | — (senza lettori dal 2026-07, rimozione in Fase 3) | Era la ricostruzione del run tessellato in un nodo bezier editabile per l'editor 2D (`group-curve-runs`/`seed->nodes`). Dopo la lettura ad alto livello (dev-docs/brief-recording-highlevel-lettura-2d.md) `seed->nodes` interpreta direttamente `:bezier-to`/`:arc-h`/`:arc-v` sui comandi alto livello (stesso pose walk di `seed->nodes-3d` + proiezione sul piano (a,b)), senza bisogno del rider |

> **Nota (Fase 2a + lettura 2D, dev-docs/brief-recording-highlevel-fase2a.md e dev-docs/brief-recording-highlevel-lettura-2d.md)**: sia il lato 3D (`seed->nodes-3d`) sia il lato 2D (`seed->nodes`/`project-2d-to-xy`) di edit-path interpretano ora i comandi alto livello del path (`:commands`) direttamente — nessuno dei due passa più da `path-micro-commands`/`:pure`/`:span`/`:arc-cap`. Il gate è chiuso: `:pure`/`:span`/`:veer-deg` non hanno più lettori in tutto `src/` (whitelist dell'editor vuota); `:arc-cap` mantiene il suo lettore in `extrusion.cljs` (lo stamping del cap in estrusione, non la lettura editor). La rimozione dell'emissione stessa è Fase 3.
