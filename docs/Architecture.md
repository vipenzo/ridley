...
`loft-between` interpola fra due shape diverse lungo un path, costruendo ring che gradualmente si trasformano dall'una all'altra. La gestione degli angoli del path, comune a tutte le varianti, sta in `process-loft-corners`, che calcola offset di scorciatoia per profili rastremati e gestisce le giunture in base al `joint-mode`. Su path con curvatura molto stretta rispetto al raggio della shape i ring possono auto-intersecarsi: `pure-loft-path` (in `editor/operations.cljs`) controlla i path bezier con `bezier-path-has-self-intersection?` e, in caso positivo, delega internamente alla funzione `bloft` di `turtle/loft.cljs`, che campiona la curvatura in modo adattivo e sostituisce le coppie di ring intersecanti con un convex hull tramite Manifold (`hull-from-points`). Per path non-bezier non c'e' fallback e l'utente deve smussare a monte (`bezier-as`, `arc-h`, raggio piu' piccolo).
