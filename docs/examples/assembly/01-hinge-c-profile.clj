...
;;
;; Section 10 adds a procedural animation that rotates the inner-C
;; around the pin, exercising the cp-f shift on inner-shell: without
;; that shift the rotation would happen around the box centre and the
;; pieces would clip through each other.
...
;; ============================================================
;; 10. ANIMATION (kinematic verification)
;; ============================================================
;; Rotates the inner-C around the pin axis. The pivot point comes
;; entirely from the inner-shell's creation-pose, which `cp-f` shifted
;; from the box centre to the pin axis (see section 6). Without that
;; shift, rotation would happen around the box centre and the inner
;; geometry would clip through the outer shell.
;;
;; Rotation axis: the pin sits along +Y (built as a cylinder along
;; heading, then `(th 90)` aligns heading to +Y). At the default turtle
;; frame, right = +Y, so `(tv angle)` rotates around +Y — i.e. around
;; the pin. `attach` rotates the mesh around the turtle position, which
;; for `hinge-inner` is the world origin (= pin-axis mark).
;;
;; Single ~6 s cycle with three holds plus an explicit return to 0°,
;; so `:loop` is seamless (no jump between iterations):
;;
;;     0°   rest        — leg parallel to the panel
;;     60°  operating   — leg tilted back as a support
;;     180° fully open  — leg flipped over
;;
;; Comment out the `(anim-proc! ...)` form to disable.
...
(defn segment
  "Eased lerp from a0 to a1 as t goes from t0 to t1."
  [t t0 t1 a0 a1]
  (let [u (ease-in-out (/ (- t t0) (- t1 t0)))]
    (+ a0 (* u (- a1 a0)))))

(defn hinge-angle
  "Piecewise schedule: hold 0° → 60° → hold 60° → 180° → hold 180° → 0°."
  [t]
  (cond
    (< t 0.15) 0
    (< t 0.35) (segment t 0.15 0.35 0   60)
    (< t 0.50) 60
    (< t 0.75) (segment t 0.50 0.75 60  180)
    (< t 0.85) 180
    :else      (segment t 0.85 1.00 180 0)))

(defn hinge-at-angle [angle]
  (mesh-union
   hinge-outer
   (attach hinge-inner (tv angle))))

(anim-proc! :open-close 6.0 :Hinge :linear :loop
            (fn [t] (hinge-at-angle (hinge-angle t))))

;; Animations register in `:stopped` state — kick playback off explicitly.
(play! :open-close)
