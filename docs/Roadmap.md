...
**Estendere `embroid` a `shell` (campo di pattern condiviso)**: `embroid` (traforo di una parete sottile già esistente) e `shell` (svuotamento di un solido in parete sottile) condividono la stessa astrazione di fondo — un campo `(fn [...] -> 0..1)` su una superficie parametrizzata, tagliato con l'isocontour. Oggi però i due campi sono separati e parametrizzati diversamente: `embroid` usa `panel-field` su `(u = ascissa lungo il path, t = sweep)` in unità fisiche, mentre `shell` usa `style->thickness-fn` su `(a = angolo atan2 attorno al centroide, t)`. Il nuovo `:style :pattern` di `embroid` (piastrellatura di un motif arbitrario via distanza con segno, con `:spacing`/`:grid`/`:inset`/`:invert?`) è il candidato naturale da promuovere a meccanismo condiviso — idealmente una `pattern-tile`/`tile-field` riusabile, esposta come `:style :pattern` su entrambi e anche come funzione standalone. Blocco di design: la parametrizzazione angolare di `shell` non è isometrica, quindi per piastrellare un motif senza distorsione sulla circonferenza serve passare la lunghezza fisica (perimetro) e mappare il motif a spaziatura reale, come già fa `embroid` con `:u-length`/`:v-length`. Va anche valutato se unificare i due builder isocontour (`build-shell-isocontour-mesh` chiuso/wrapping vs `build-embroid-mesh` aperto/non-wrapping) o tenerli distinti. Tre-cinque giorni, da fare dopo il marching squares (che tocca comunque gli stessi builder).
