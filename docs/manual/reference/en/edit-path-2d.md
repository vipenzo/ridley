...
(This edits a **planar 2D profile**. For editing a **3D rail** — a path consumed by
extrude-along-path / loft in its own frame — use the separate **edit-path** instead.)
...
- **Related:** `edit-path`, `path-2d`, `image-board`, `edit-image-board`, `set-image`,
  `path-to-shape`, `shape-intersection`, `edit-bezier`, `move-to`
