...
- **2026-06-19 (cap. 3: workflow ricalco da foto + convenzione rimandi a sezione)** — *Documentato il ricalco di un contorno da foto e fissata la convenzione per i rimandi interni.* Nuova estensione di Ridley (Vincenzo+Code) per ricalcare profili reali da immagini: `image-board` (board di tracciamento con foto calibrata a misura reale, wrapper su `set-image` di un rect `preserve-position?`, tartaruga ferma a `[0 0]`), `edit-image-board` (editor modale per calibrare e inquadrare), `path-2d` (path planare distinto-ma-compatibile, `:species :2d`, non-breaking) e `edit-path-2d` (pen tool modale che cuoce un `path-2d`); `edit-path` resta l'editor del rail 3D. Schede reference già a posto, con correzioni residue in carico a Code (esempio di `edit-path-2d` da `set-image` a `image-board`; riposizionamento di `set-image` come primitiva di basso livello). Documentazione nel cap. 3 come **due sottosezioni `###` in coda a «Profili come valori»** (non sezioni `##`: scelta deliberata per non rinumerare e non rompere i rimandi a numero di sezione): «Dove atterra il profilo: l'ancoraggio» (paga il rimando sospeso di `make-shape` nella sezione «Forme personalizzate», spiega l'ancoraggio centrato / primo-vertice / `preserve-position?` osservabile con `stamp`, due esempi eseguibili) e «Ricalcare un contorno da una foto» (cenno `:no-run :warning desktop-only`, arco image-board → edit-path-2d → `path-to-shape … :preserve-position` → extrude). Casa di `:preserve-position` decisa al livello di `stamp`, non della creation-pose: è un solo flag (l'attributo shape `:preserve-position?`, di cui l'opzione di `path-to-shape`/`stroke-shape` è scorciatoia), onorato da `stamp` ed `extrude` e, dopo il fix di Code, anche dal loft a due shape (resta fuori solo `revolve`, per natura radiale); la creation-pose avrà un rimando all'indietro quando si scriverà la sua sezione (cap. 4 o 8). **Convenzione rimandi adottata**: i numeri di capitolo si possono citare (i titoli di capitolo nel renderer sono numerati, es. "5. Path"), i numeri di sezione no (le testate di sezione non lo sono). Regola: sezione vicina → riferimento relativo ("più sopra", "qui sotto", "la sezione precedente"); sezione lontana nello stesso capitolo → per «titolo» (caporali); altro capitolo → per numero di capitolo. Applicata al cap. 3, cinque rimandi bonificati (§3.7 → «Dove si usano le shape»; "nella 3.5" → "più sopra"; "(3.1-3.4)/(3.5-3.6)" rimossi; "nella 3.4" → «Profili come valori»; "sezione 7.5" → "lì"). Bonifica degli altri capitoli rinviata, da fare man mano che si toccano. Tutte le modifiche speculari IT+EN; `structure.cljs` e `manual_levels` invariati (sezioni interne a pagina già bilingue, nessun marker di capitolo toccato). **Pendenti**: sezione `path-2d` nel cap. 5 (perché il rimando "materiale dei path (cap. 5)" atterri su qualcosa di scritto); rimando all'indietro creation-pose ↔ `preserve-position?`; verifica nel renderer che i due `stamp` dell'ancoraggio mostrino la differenza (visibilità origine/tartaruga) e unicità globale degli id esempio nuovi (`anchoring-first-vertex`, `anchoring-preserve-position`, `trace-from-photo`); lato Code, test di regressione del fix loft a due shape e voce in `code-issues.md` sulla trappola degli attributi hardcoded in `lerp-shape`/`make-shape`.
