...
## Esito Fase 2 (2026-07-07) — lettura 2D, gate chiuso

Atterrato l'ultimo brief della Fase 2 (`dev-docs/brief-recording-highlevel-lettura-2d.md`, il gemello 2D della Parte 2 di Fase 2a): `seed->nodes`/`project-2d-to-xy` (editor 2D, `edit_path.cljs`) riscritti come interprete diretto sui comandi alto livello — stesso pose walk di `seed->nodes-3d` (reduce che threada `{pos heading up}` in 3D, `turtle/local->world` + `canonical-bezier-frame` per i bezier) seguito da una proiezione sul piano (a,b), invece di ricostruire i nodi dalla tessellazione. `group-curve-runs`/`walk-arc-sub`/`in-plane-cmd?` rimossi. Un `:bezier-to` alto livello è direttamente un nodo bezier; un `:arc-h`/`:arc-v` alto livello è un nodo arco con raggio/sweep letti dagli argomenti (closed form dedicato, `arc-2d-endpoint`, verificato via nREPL contro `turtle/arc-v` diretto — NON lo stesso `arc-v-endpoint` del lato 3D, la cui formula per `:end` disaccorda da `turtle/arc-v` per un segno: bug preesistente, fuori scope, non toccato). L'hack dell'offset sul rider dei bezier (move-to non applicato dal recorder) sparisce da solo: `:move-to` è threadato nella posa. Suite verde (439 test / 1344 assert / 0 failures), nuovo corpus di round-trip in `test/ridley/editor/edit_path_2d_test.cljs` (dritti, archi, bezier, il caso storicamente hackerato move-to+bezier-che-flippa-la-normale-in-piano, side-trip con curva, mark, path chiuso) verificato anche per equivalenza diretta con l'implementazione precedente via nREPL (byte-identico a rumore in virgola mobile).

**Deviazione enumerata:** i nodi arco riaperti recuperano raggio/sweep esatti dagli argomenti del comando invece di ricostruirli dalla tessellazione — il punto fisso bake→confirm→riapertura è più preciso di prima (verificato contro una tessellazione a 10000 passi di `turtle/arc-v`), mai peggiore.
