...
(defn walk-path-poses
  ...
  (let [commands (vec (:commands path))
        ...
        total-dist (reduce (fn [acc cmd]
                             (if (= :f (:cmd cmd))
                               (+ acc (Math/abs (double (first (:args cmd)))))
                               acc))
                           0 commands)
    ...
    (loop [i 0, s init, prev-h nil, total-dist 0.0, total-angle 0.0]
      (if (>= i n)
        ...
        (let [cmd (nth commands i)]
          (cond
            (= :f (:cmd cmd))
            (let [d (Math/abs (double (first (:args cmd))))
                  h (:heading s)
...
(defn- calc-bloft-steps
  ...
  (let [{:keys [mode value]} (get-resolution state)]
    (case mode
      :n (max 2 (int (+ (* value (/ total-angle 360)) (* (+ 1 (Math/log (double value))) total-dist))))
  ...
  ([state shape transform-fn bezier-path steps threshold]
   (if-not (and (shape? shape) (is-path? bezier-path))
     ...
     (let [creation-pose {:position (:position state)
           ...
           _ (println "bloft scan:" "total-angle=" (format "%.1f" (double (:total-angle scan)))
                      "total-dist=" (format "%.1f" (double (:total-dist scan)))
                      "radius=" (format "%.2f" (double initial-radius))
                      "steps=" steps)
       ...
       (if (< n-poses 2)
         ...
         (let [result
               ...
               _ (println "bloft: threshold=" threshold
                          "shape-radius=" (format "%.2f" (double initial-radius))
           ...
           (if (empty? final-meshes)
             ...
             (let [unified-mesh (if (= 1 (count final-meshes))
                                  ...
                                  (try
                                    ...
                                    (catch Exception e
                                      (println "bloft: manifold union failed, concatenating meshes:" (.getMessage e))
