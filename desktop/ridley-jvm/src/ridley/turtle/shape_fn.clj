          ...
          (let [x (* (/ (+ ix 0.5) w) size)
                ...
                weft-z (when (< dx radius)
                         (+ (* lift
                            ...
                            (prof dx)))
                z (cond
                    (and warp-z weft-z) (max warp-z weft-z)
                    warp-z warp-z
                    weft-z weft-z
                    :else z-floor)]
            (aset ^doubles data (+ ix (* iy w)) (double z))))))
    ;; Normalize to [0,1]
    (let [z-min (loop [i 0 m Double/POSITIVE_INFINITY]
                  (if (>= i n) m
                      (recur (inc i) (min m (aget ^doubles data i)))))
          z-max (loop [i 0 m Double/NEGATIVE_INFINITY]
                  (if (>= i n) m
                      (recur (inc i) (max m (aget ^doubles data i)))))
          z-range (- z-max z-min)]
      (when (> z-range 0)
        (dotimes [i n]
          (aset ^doubles data i (/ (- (aget ^doubles data i) z-min) z-range))))
      {:type :heightmap
       :data data
       :width w :height h
...
(defn- rasterize-triangle!
  ...
  [^doubles data grid-w grid-h x-min y-min sx sy v0 v1 v2]
      ...
      (let [px (+ x-min (* (+ gx 0.5) sx))
        ...
        (when (and (>= bu 0) (>= bv 0) (>= bw 0))
          (let [z (+ (* bu (nth v0 2)) (* bv (nth v1 2)) (* bw (nth v2 2)))
                idx (+ gx (* gy grid-w))]
            (when (> z (aget ^doubles data idx))
              (aset ^doubles data idx (double z)))))))))

(defn mesh-to-heightmap
  ...
  (let [verts (:vertices mesh)
        ...
        h resolution
        n (* w h)
        data (double-array n Double/NEGATIVE_INFINITY)
        sx (/ (- x-max x-min) w)
        sy (/ (- y-max y-min) h)]
    ...
    (let [z-min (loop [i 0 m Double/POSITIVE_INFINITY]
                  (if (>= i n) m
                      (let [v (aget ^doubles data i)]
                        (recur (inc i) (if (Double/isFinite v) (min m v) m)))))
          z-max (loop [i 0 m Double/NEGATIVE_INFINITY]
                  (if (>= i n) m
                      (let [v (aget ^doubles data i)]
                        (recur (inc i) (if (Double/isFinite v) (max m v) m)))))
          z-range (- z-max z-min)]
      (if (> z-range 0)
        (dotimes [i n]
          (let [v (aget ^doubles data i)]
            (aset ^doubles data i (double (if (Double/isFinite v)
                                            (/ (- v z-min) z-range)
                                            0)))))
        ;; z-range = 0: binary heightmap (finite -> 1, -Inf -> 0)
        (dotimes [i n]
          (let [v (aget ^doubles data i)]
            (aset ^doubles data i (double (if (Double/isFinite v) 1.0 0))))))
      {:type :heightmap
       :data data
       :width w :height h
...
(defn- voronoi-hash
  "Deterministic pseudo-random 2D hash based on cell coordinates and seed.
   Returns [x y] in [0,1)x[0,1) -- the jittered cell center."
  [ix iy seed]
...
(defn- style->thickness-fn
  ...
  (case style
    ...
    ;; Unknown style
    (throw (Exception. (str "shell: unknown :style " style
