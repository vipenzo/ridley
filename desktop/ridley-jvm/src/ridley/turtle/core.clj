...
(def build-segment-mesh extrusion/build-segment-mesh)
(def build-corner-mesh extrusion/build-corner-mesh)
(def shape-radius extrusion/shape-radius)
(def compute-stamp-transform extrusion/compute-stamp-transform)
(def transform-2d-to-3d extrusion/transform-2d-to-3d)
(def stamp-shape extrusion/stamp-shape)
(def stamp-shape-with-holes extrusion/stamp-shape-with-holes)
(def earcut-triangulate extrusion/earcut-triangulate)
(def project-to-2d extrusion/project-to-2d)
...
(def rotate-ring-around-axis extrusion/rotate-ring-around-axis)
(def scale-ring-from-centroid extrusion/scale-ring-from-centroid)
(def scale-ring-along-direction extrusion/scale-ring-along-direction)
...
(def sweep-two-shapes-with-holes extrusion/sweep-two-shapes-with-holes)
(def sweep-two-shapes extrusion/sweep-two-shapes)
...
(def analyze-closed-path extrusion/analyze-closed-path)
(def analyze-open-path extrusion/analyze-open-path)
(def is-simple-forward-path? extrusion/is-simple-forward-path?)
(def extrude-from-path extrusion/extrude-from-path)
(def extrude-closed-from-path extrusion/extrude-closed-from-path)
...
(def stamp-loft loft/stamp-loft)
(def finalize-loft loft/finalize-loft)
(def analyze-loft-path loft/analyze-loft-path)
(def loft-from-path loft/loft-from-path)
(def bloft loft/bloft)

;; --- Re-exports from attachment module (facade pattern) ---

(def mesh-centroid attachment/mesh-centroid)
(def translate-mesh attachment/translate-mesh)
(def rotate-mesh attachment/rotate-mesh)
(def scale-mesh attachment/scale-mesh)
...
(def extract-face-perimeter attachment/extract-face-perimeter)
(def build-face-extrusion attachment/build-face-extrusion)
...
(def build-face-inset attachment/build-face-inset)
...
(def inset attachment/inset)
(def build-face-scale attachment/build-face-scale)
(def scale-attached-face attachment/scale-attached-face)
(def scale attachment/scale)
(def compute-triangle-normal attachment/compute-triangle-normal)
(def compute-face-info-internal attachment/compute-face-info-internal)
(def clone-mesh attachment/clone-mesh)
(def attached? attachment/attached?)

;; --- Re-exports from bezier module (facade pattern) ---

(def cubic-bezier-point bezier/cubic-bezier-point)
(def quadratic-bezier-point bezier/quadratic-bezier-point)
(def cubic-bezier-tangent bezier/cubic-bezier-tangent)
(def quadratic-bezier-tangent bezier/quadratic-bezier-tangent)
(def auto-control-points bezier/auto-control-points)
(def auto-control-points-with-target-heading bezier/auto-control-points-with-target-heading)
(def compute-bezier-control-points bezier/compute-bezier-control-points)
(def sample-bezier-segment bezier/sample-bezier-segment)
...
(defn move-lateral
  ...
  (when (#{:shape :loft} (:pen-mode state))
    (throw (Exception. "Lateral movement (u/d/rt/lt) is not allowed inside extrude/loft")))
...
;; ── Recording bezier-as (for path macro) ────────────────────────

(defn- compute-rotation-angles
  "Convert an absolute direction vector to relative [th-deg tv-deg] rotations
   from the current heading/up frame."
  ...
  (let [up-comp (dot to-direction from-up)
        ...
        tv-deg (* (Math/atan2 up-comp horiz-len) (/ 180 Math/PI))
        th-deg (if (> horiz-len 0.001)
                 (let [horiz-norm (normalize horiz-dir)
                   ...
                   (* (- (Math/atan2 right-comp fwd-comp)) (/ 180 Math/PI)))
                 0.0)]
    [th-deg tv-deg]))

(defn rec-bezier-as
  "Recording version of bezier-as. Decomposes a bezier approximation of a path
   into f/th/tv commands and records them. Preserves :mark commands from the
   original path segments. Works with the immutable recorder state."
  [state p & args]
  (let [{:keys [tension steps max-segment-length cubic]} (apply hash-map args)
        segments (path-segments p)
        segments (if max-segment-length
    ...
    (if (empty? segments)
      state
      (let [init-pose (select-keys state [:position :heading :up])
            calc-steps-fn (fn [_] (or steps 16))
            walk-data (compute-bezier-walk
                       segments init-pose
                       {:tension (or tension 0.33)
                        :cubic cubic
        ...
        (reduce
         (fn [s [seg segment-data]]
            ;; Insert any marks from this segment's rotations
           (let [marks (filter #(= :mark (:cmd %)) (:rotations seg))
                 s (reduce (fn [st m] (update st :recording conj m)) s marks)]
             (if (:degenerate segment-data)
               s
               (reduce
                (fn [s2 step]
                  (let [{:keys [dist chord-heading final-heading]} step
                        cur-heading (:heading s2)
                        cur-up (:up s2)
                        [th-a tv-a] (compute-rotation-angles cur-heading cur-up chord-heading)
                        s3 (cond-> s2
                             (> (Math/abs th-a) 0.001) (rec-th th-a)
                             (> (Math/abs tv-a) 0.001) (rec-tv tv-a))
                        s4 (rec-f s3 dist)
                        cur-heading2 (:heading s4)
                        cur-up2 (:up s4)
                        [th-b tv-b] (compute-rotation-angles cur-heading2 cur-up2 final-heading)
                        s5 (cond-> s4
                             (> (Math/abs th-b) 0.001) (rec-th th-b)
                             (> (Math/abs tv-b) 0.001) (rec-tv tv-b))]
                    s5))
                s
                (:walk-steps segment-data)))))
         state
         (map vector segments walk-data))))))
